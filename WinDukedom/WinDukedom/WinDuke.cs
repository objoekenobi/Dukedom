...
{
	public partial class WinDuke : Form
	{
		...
		public void RunGame()
		{
			while (CheckGame())
			{
				...
				if (Functions.FNR(1, 6) < WarChance)
				{
					...
					if (Result == War.Victory.CompleteWin)
					{
						if (KingStatus == 0)
						{
					...
					else if (Result == War.Victory.SimpleWin)
					{
						population.WarCasualties = war.DeathToll;
						grain.MercenaryHire = war.GrainCost;
						grain.FruitsOfWar = war.SoWGrain;
					}
					else if (Result == War.Victory.Draw)
					{
					}
					else if (Result == War.Victory.SimpleLoss)
					{
						population.WarCasualties = war.DeathToll;
						grain.MercenaryHire = war.GrainCost;
						grain.FruitsOfWar = war.SoWGrain;
					}
					else if (Result == War.Victory.CompleteLoss)
					{
						Dead = true;
					}
				}
